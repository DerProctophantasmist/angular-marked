...
function markedProvider() {
   ...
   */
markedDirective.$inject = ['marked', '$templateRequest', '$compile', 'Locale'];
function markedDirective(marked, $templateRequest, $compile, Locale) {
  return {
    restrict: 'AE',
    replace: true,
    scope: {
    ...
    link: function (scope, element, attrs) {
            ...
            var getContent = function (filename) {
                $templateRequest(filename, true).then(function (response) {
                    set(response);
                }, function () {
                    set('');
                    scope.$emit('$markedIncludeError', filename);
                });
            };
      if (attrs.marked) {
        set(scope.marked);
